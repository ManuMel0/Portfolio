...
(function($) {
   ...
   $(window).load(function() {
	...
	/*---------------------------------------------------- */
	/* Owl Carousel Idiomas
	------------------------------------------------------ */
	$("#owl-slider-idiom").owlCarousel({
        navigation: false,
        pagination: true,
        itemsCustom : [
	        [0, 1],
   ...
   statSection.waypoint({
   	...
   	handler: function(direction) {
      	...
      	if (direction === "down") {       		
			   ...
			   stats.each(function () {
				   var $this = $(this);
				   var rawValue = $.trim($this.text());
				   var normalizedValue = rawValue.replace(',', '.');
				   var isNumericValue = /^-?\d+(\.\d+)?$/.test(normalizedValue);

				   if (!isNumericValue) {
					   return;
				   }

				   $({ Counter: 0 }).animate({ Counter: parseFloat(normalizedValue) }, {
			   		duration: 4000,
			   		easing: 'swing',
